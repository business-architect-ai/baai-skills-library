...
## ⚠️ NOT for Facebook/Instagram ad creatives

If the brief asks for a Facebook/Instagram/Meta ad creative (1080×1080 Feed, 1080×1920 Story, 1.91:1 link ad), STOP and switch to:

- `[[image-ads]]` — KIE GPT Image 2 with pose-matched speaker photos
- `[[creative-pipeline]]` — automated batch from `.md` ad copy file (KIE + logo overlay + manifest)

Huashu-design is for HTML mockups, slides, prototypes, animations — NOT for paid social creatives. Using it there bypasses brand pipeline, logo overlay, and Drive upload.

**Exception:** placeholder/demo cases with NO real brand assets (e.g., a demo for a fictional client) — huashu is acceptable. Always disclose in war-room submission: "huashu used because no brand pipeline applies for fictional demo."
...
## Playwright 1080×1080 screenshot (reusable snippet)

For square deliverables (ad mockups, story cards):

```bash
npx playwright screenshot "file://${HTML}" "${OUT}.png" \
  --viewport-size=1080,1080 \
  --full-page=false
```

For 1080×1920 (Story) swap viewport. Always verify with `identify` (ImageMagick) or `sips` to confirm dimensions before war-room submission.

After screenshot, run `[[validate-creatives]]` for diacritics/colors/dimension checks BEFORE submitting to war-room.
...
**人物照片专项预检**（涉及真人头像/身像时强制执行）：

从照片库挑人物素材时，**渲染前**先用肉眼判断脸部位置：

| 脸部位置 | 适合的容器 |
|---|---|
| 上三分之一（portrait close-up，背景留白多在下方） | 1:1 / 4:5 圆角卡，`object-position: center 18%` |
| 中三分之一（half-body / 业务半身像） | 3:4 / 4:5 卡，`object-position: center 30%` |
| 下三分之一 / 远景 | **换照片**——这种构图不适合 hero 卡，再多 CSS 调位也救不回 |

**经验法则**：如果你打算用 `object-position: center 70%+` 才能露脸，那你选错了照片，不是写错了 CSS。
...
##### 2-strike 照片迭代规则（用户反馈型工作流）

用户反馈「照片不居中 / 没露脸 / 太小 / 看不见人」时，**第 1 次尝试**：调 `object-position` 或 `object-fit`。**第 2 次反馈仍不对**：**停止 CSS 调试，换照片文件**。

理由：CSS `object-position` 只能在原图框内平移裁切，无法改变脸部在原图的相对位置。如果原图脸部在上三分之一而你的卡片是 1:1，调到极限也只能 `center 18%`——再往上就是黑边。第 3+ 次还在调 CSS，是在用错误的工具解决资产选择问题。

**真实成本对比**（2026-05-07 Manifest landing 实测）：
- 走 2-strike 规则：最多 2 次 CSS 调整 + 1 次 swap photo = 3 次迭代收敛
- 不走规则：65 个 photo-related tool calls，9 版 CSS 才换照片，浪费 ~30 分钟用户来回

**正确反应模板**：
> 「这个照片的脸部在原图上三分之一，我刚才用 `object-position: center 18%` 是裁切极限。再调没空间了。我建议换 `<其他文件名>` —— 那张是中心构图，1:1 卡片直接居中。要换吗？」

## Tool harness conventions（Claude Code 专属）

在 Claude Code 里调 huashu-design 时，记住：

- **Edit / Write 之前必须先 Read 目标文件**——否则 harness 报错「File has not been read yet. Read it first before writing to it.」。新建文件除外（Write 一个不存在的文件 OK）。
- **Edit 的 `old_string` 必须在文件中唯一**——拷贝足够多的上下文行让它唯一。匹配失败会报 `String to replace not found in file`。
- **绝对路径优先**——所有 Read/Write/Edit 用绝对路径，不用相对路径（cwd 可能在子任务间改变）。
- **Bash 不要用来 cat/head/tail/sed 文件**——用 Read/Edit 直接做，更快也避免误操作。
- **`open <path>` 在 macOS 上无输出是正常的**——不要因此重试或假设失败。
...
## Landing Page Production Shortcut: `landing-wow-builder-v2`

> **When huashu-design is acting as the PRIMARY landing-page skill in the al-* agent team** (used by `al-web-designer`), it has access to a secondary execution-layer skill specialized in fast preset application: `landing-wow-builder-v2`.

### When to delegate to landing-wow-builder-v2

After the design direction is approved (your advisor mode, junior designer mode, or expert review confirmed it), if the task is:
- Production landing page (not prototype, not exploration, not animation-heavy)
- Copy is finalized
- Preset is chosen (or matches one of v2's 8 named presets)
- Need fast template fill + responsive boilerplate

→ Invoke v2 from inside huashu workflow:
```
Skill(skill: "landing-wow-builder-v2", args: "--preset <name> --layout <type> --copy-source <path>")
```

### Skill location (al-* agent team only)

```
domains/asistent-lansare/.claude/skills/landing-wow-builder-v2/
├── SKILL.md                          # Step-by-step workflow
├── visual-picker.html                # All effects on one page (preview)
├── preview/                          # 8 preset HTMLs (Dark Cyberpunk / Tech / Editorial / Light Editorial / Warm Friendly / Glassmorphism Premium / Luxury Minimal / Neo-Brutalism)
├── templates/full-pages/             # Webinar, lead-gen, masterclass templates with {{PLACEHOLDER}}
└── references/                       # KEY library files referenced from huashu
    ├── effects-library.md            # 20+ CSS/JS effects (glow, 3D-tilt, countdown, particles)
    ├── color-schemes.md              # 6 named palettes
    ├── typography-guide.md           # Font pairings per preset
    ├── landing-components.md         # Hero / pricing / FAQ / countdown components
    ├── mobile-optimization.md        # 375px base, sticky CTA, safe-area insets
    ├── template-matrix.md            # Awareness level → layout mapping (Eugene Schwartz)
    └── presets.json                  # Per-preset config
```

### Decision rule (huashu primary, v2 secondary)

| Scenario | Skill | Why |
|---|---|---|
| Vague brief, 3 directions wanted | huashu (advisor) | Strategy + philosophy belongs to huashu |
| Approved direction, custom landing | huashu (junior designer) | Iteration + variants belongs to huashu |
| Approved copy + preset → producție HTML | huashu **delegates to v2** | v2 owns fast template fill |
| Animated landing hero | huashu (uses v2's effects-library.md as reference) | Motion belongs to huashu, effects sourced from v2 |
| MP4/GIF export of landing | huashu only | v2 doesn't export video |
| iOS app prototype | huashu only | v2 is landing-only |
| Slide deck with motion | huashu only | v2 is landing-only |
| Final expert review | huashu | 5-dimension audit belongs to huashu |

### What v2 brings to huashu workflow

- **Speed:** template fill in minutes vs hours of custom design
- **Mobile-first boilerplate:** 375px base + breakpoints + safe-area insets pre-baked
- **Named presets:** 8 ready-to-apply styles
- **Effects library:** countdown timers, count-up animations, particle backgrounds, 3D-tilt cards (referenceable inside huashu's animations too)
- **Awareness-level mapping:** Eugene Schwartz framework → layout type

What v2 does NOT bring (huashu retains these):
- Design strategy / philosophy / advisor mode
- Tweaks system (component variant switcher)
- Animation philosophy + best practices
- App mockups / iOS frames / AppPhone state manager
- Video export (MP4/GIF)
- Expert review on 5 dimensions
- Brand asset protocol (logo + product image research)

### Detailed integration guide

See `references/landing-integration.md` for full decision tree, scenario matrix, shared assets reference, and handoff protocol between huashu advisor → v2 template fill.

### Bypass note

If the al-* pipeline yaml passes `bypass_huashu: true` in ARGS_JSON, the agent skips huashu entirely and goes straight to v2. Power-user shortcut for mechanical re-renders. Default: NEVER bypass.
